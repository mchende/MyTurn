...
## 0. Phase 1 Prototype Authority

Phase 1 的首页与课堂页现以 Stitch 原型为正式落地依据，原型文件位于 `UI/stitch_kids_online_classroom_design/`。当这些页面原型与下文的通用 Apple-inspired 基线冲突时，**以原型页面为准**。

### Authoritative Prototype Files

- `UI/stitch_kids_online_classroom_design/screen-主页.png`
- `UI/stitch_kids_online_classroom_design/code-主页.html`
- `UI/stitch_kids_online_classroom_design/screen-课堂页.png`
- `UI/stitch_kids_online_classroom_design/code-课堂页.html`

### Scope Note

- `screen-设置页.png` 与 `code-设置页.html` 仅作为未来配置后台的参考，不纳入当前 Phase 1 交付范围。

### Phase 1 Overrides

- 首页采用 Stitch 原型定义的“左侧轻导航 + 右侧课程卡片网格”结构，而不是纯单列课表页。
- 首页主标题使用 `我的课堂`，保留班级入口感；实现时允许将原型中的按钮文案 `上课` 语义化收敛为 `进入课堂`，以匹配入场窗口规则。
- 课堂页采用 Stitch 原型定义的“顶部学生席条 + 左侧深色课件主舞台 + 右侧讲台/教师双卡片 + 底部控制条”结构。
- 视觉语气以 Stitch 的 Academic Sanctuary 方向为准：浅底卡片、克制绿作为主强调色、圆角和轻阴影共存，不再强制执行 Apple Blue 作为唯一强调色。
- 字体语气以 `Plus Jakarta Sans` 标题和 `Manrope`/兼容系统字体正文为优先参考；若运行环境不具备对应字体，使用最接近的 system fallback。

### Practical Reading Order

实现 Phase 1 前，先读原型截图与 HTML，再读本文其余章节作为补充原则。对本 phase 来说，页面结构、信息优先级、卡片布局、课堂分区和 CTA 位置，优先从原型中抽取，不从下文泛化推断。
