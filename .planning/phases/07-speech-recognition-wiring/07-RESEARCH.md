...
**Fresh source correction:** 官方 MDN 文档现在明确把 `SpeechRecognition` 和 `stop()` 都标成 “Limited availability”，说明它们不是稳定 Baseline 能力；同时 MDN 还明确指出，像 Chrome 这类浏览器在某些情况下会使用服务端识别引擎，音频会发往 web service，因此离线不可用。这意味着 Phase 7 不能把 browser-native recognition 当成“所有主流浏览器都稳定可用且本地完成”的前提，而必须把 `unavailable`、`network-like failure` 和 provider variability 当成一等失败路径。
...
| Browser speech recognition adapter (`SpeechRecognition` / `webkitSpeechRecognition`) | MVP transcript 来源 | 不新增后端和第三方 SDK 时，浏览器原生 adapter 是最低摩擦的 transcript provider，但必须显式处理 “Limited availability” 和 provider 缺失 |
...
**Fresh source constraint:** 官方 API 明确给出 `continuous`、`interimResults` 和 `maxAlternatives` 这些 recognition 行为开关。对 MyTurn 这种“孩子说一句 -> 等 final transcript -> 继续课堂”的模式，研究结论是：
- `continuous = false`
- `interimResults = false`
- `maxAlternatives = 1`
这样最贴合当前单 CTA、单次判断的课堂 contract，避免引入半成品 interim transcript 抖动 UI。
...
**Fresh source constraint:** MDN 对 `SpeechRecognition.stop()` 的描述是“停止监听并尝试基于目前已捕获结果返回结果”。这正好支持当前产品选择的纯点按式 stop 语义：第二次点击 CTA 不是“简单停录音”，而是“显式结束当前听写窗口，并尝试产出 final recognition result”。
...
**Fresh source constraint:** `result` 事件提供 `SpeechRecognitionEvent.results` 和 `resultIndex`，`error` 事件提供 `SpeechRecognitionErrorEvent.error` 与 `message`。因此 telemetry 不应该只记录“成了/没成”，而应至少区分：
- final transcript resolved
- explicit error event
- timeout without final result
- empty transcript despite stop
...
**Fresh source constraint:** 既然官方文档已经明确 `SpeechRecognition` 不是 Baseline，而且部分浏览器会走服务端识别引擎，focused e2e 就绝不能依赖真实 provider 在线可用性。fake provider 不是“测试偷懒”，而是对浏览器能力波动的必要隔离。
...
- **默认依赖 interim transcript：** 官方文档明确 `interimResults` 默认是 `false`；对当前课堂场景，强行把 interim transcript 推上 UI 只会增加抖动和误导。
...
### Pitfall 5: 把 browser-native recognition 当作离线本地能力
**What goes wrong:** 设计和验收都假设“浏览器自己本地识别”，但真实目标浏览器可能把音频送到服务端识别，离线时直接不可用。
**How to avoid:** 把 provider variability 明确放进 research 和 runtime reason 枚举，自动化全部走 fake provider，真实浏览器只做人手验证。

## Code Examples

Verified patterns from official sources and current codebase:

### Browser Recognition Should Start From An Explicit User Action
```ts
...
### Final Transcript Comes From The `result` Event
```ts
...
### Recognition Failures Should Be Routed Through The `error` Event
```ts
...
### Stop Should End Listening And Try To Return A Result
```ts
recognition.stop();
```
...
- Workspace inspection:
 ...
 - Official docs:
  ...
  - MDN `SpeechRecognition.result event`: https://developer.mozilla.org/en-US/docs/Web/API/SpeechRecognition/result_event
  - MDN `SpeechRecognition.error event`: https://developer.mozilla.org/en-US/docs/Web/API/SpeechRecognition/error_event
  - MDN `SpeechRecognition.stop()`: https://developer.mozilla.org/en-US/docs/Web/API/SpeechRecognition/stop
...
- Browser speech recognition support variability beyond the official MDN “Limited availability” marker still needs target-browser manual validation during execution.
